...
if submit:
    if not name or not email or not education:
        ...
        with st.spinner("Generating your resume..."):
            # Prompt Gemini
            response = client.generate_content(model = "models/gemini-1.5-flash", content = prompt +"\n\n"+ st.form("resume_form"))
            if response:
                st.write("✅ Resume generated successfully!")
                st.text_area("✍️ Your Resume", response.text, height=500)
            else:
...
#                 st.error("❌ Failed to generate resume. Please try again later.")